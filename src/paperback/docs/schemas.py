...
class AnalyzeLexics(BaseModel):
    docs_and_corpuses: list[DocsAndCorpuses] = Field(
        ..., description="list of documents and corpuses to analyze"
    )
    dicts: list[str] = Field(
        ..., description="list of dictionary names to use during analysis"
    )
...
class AnalyzeCompare(BaseModel):
    first_docs_and_corpuses: list[DocsAndCorpuses] = Field(
        ..., description="first list of documents and corpuses to analyze"
    )
    second_docs_and_corpuses: list[DocsAndCorpuses] = Field(
        ..., description="second list of documents and corpuses to analyze"
    )
    statistics: list[str]
