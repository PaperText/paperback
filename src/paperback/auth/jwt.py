...
def read_keys(curve: str, private_key_file: Path, public_key_file: Path) -> tuple[bytes, bytes]:
    match curve:
        case "secp521r1":
            logger.debug("reading secp521r1 keys")
            sk: ecdsa.SigningKey = ecdsa.SigningKey.from_pem(private_key_file.read_text())
            vk: ecdsa.VerifyingKey = sk.verifying_key
            sk_bytes: bytes = bytes(sk.to_pem())
            vk_bytes: bytes = bytes(vk.to_pem())
            return sk_bytes, vk_bytes
...
def get_decode_token(settings: AuthSettings = Depends(get_auth_settings), session=Depends(get_session)) -> Callable[[str], schemas.Token]:
    logger.debug(settings)
    private_key_file, public_key_file = settings.storage_path / "private.pem", settings.storage_path / "public.pem"
    private_key: bytes
    public_key: bytes
    ...
    if settings.recreate_keys:
        ...
        private_key, public_key = read_keys(settings.curve, private_key_file, public_key_file)

    def decode_token(token: str) -> schemas.Token:
        try:
            claims = jwt.decode(token, public_key, claims_options=claim_option)
            claims.validate()
